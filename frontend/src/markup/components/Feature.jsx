...
const icons = [
  "bi bi-binoculars", // Mission
  "bi bi-box-seam", // Vision
  "bi bi-brightness-high", // Core Values
  "bi bi-command", // Why we are different
];
  ...
  useEffect(() => {
    const loadFeatures = async () => {
      try {
        ...
        const formatted = response.data?.map((f, index) => ({
          id: index + 1,
          title: f.key_name
            .split("_")
            .map((w) => w[0].toUpperCase() + w.slice(1))
            .join(" "), // format "core_values" -> "Core Values"
          icon: icons[index] || "bi bi-question-circle", // fallback icon
          content: f.content,
          list: f.items?.map((i) => i.text) || undefined,
          type: f.media_type || "image",
          media: f.media_url || undefined,
        }));

        setTabs(formatted);
        setActiveTab(1); // set first tab active
      } catch (err) {
            ...
            >
              <div className="row">
                ...
                <div className="col-lg-6 order-1 order-lg-2 text-center">
                  {tab?.type === "video" ? (
                    <video src={tab.media} controls className="img-fluid" />
                  ) : (
                    <img src={tab.media} alt="" className="img-fluid" />
                  )}
                </div>
              </div>
            </div>
